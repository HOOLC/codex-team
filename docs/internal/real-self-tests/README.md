...
`real self-tests` (`真实自测`) are replayable developer-run acceptance checks against the built `codexm` CLI and TUI in an isolated runtime. They are distinct from unit tests, integration tests, and device-specific "real machine" testing: the point is to exercise the shipped operator surface the way a user would, while still keeping the operator's live runtime untouched.

## When To Run

- Inner-loop development: optional. Use automated tests first while iterating.
- Before pushing or updating a PR that changes user-visible CLI, TUI, proxy, daemon, watch, run, share-bundle, or Desktop behavior: run the smallest relevant real self-test subset that covers every touched feature area.
- When a change crosses multiple surfaces or changes shared presentation/runtime wiring, run the full core suite in [minimal-suite.md](./minimal-suite.md).
- For static config, generated docs, or non-behavioral refactors, real self-tests are not required unless the operator workflow or output contract changed.
...
```bash
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/codexm-real-selftest.XXXXXX")"
...
- Use a non-default `CODEXM_PROXY_PORT` so self-tests cannot collide with the live shared daemon/proxy.
...
- [minimal-suite.md](./minimal-suite.md): the smallest durable case set that collectively covers the main operator surfaces
