...
- `codexm proxy enable` writes a synthetic ChatGPT auth plus proxy-backed `chatgpt_base_url` and `openai_base_url` into the default `CODEX_HOME`, while keeping the live provider identity unchanged so proxy and non-proxy sessions keep shared history. Managed Desktop or CLI entrypoints launched through `codexm` route both live Responses websocket turns and REST traffic through the local proxy. The dashboard and `codexm list` always keep the synthetic `proxy` row visible; the `@` marker and `Last upstream: ...` details only appear while proxy mode is enabled and recent proxy traffic has selected a real upstream account. Bare Desktop or CLI launches outside `codexm` are not forced into that path.
