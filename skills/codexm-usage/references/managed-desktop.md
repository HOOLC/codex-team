...
- `codexm proxy enable` writes a synthetic ChatGPT auth plus proxy-backed `chatgpt_base_url` and `openai_base_url` into the default `CODEX_HOME`, while keeping the live provider identity unchanged so proxy and non-proxy sessions keep shared history. Managed Desktop or CLI entrypoints launched through `codexm` route both live Responses websocket turns and REST traffic through the local proxy. When recent proxy traffic has selected a real upstream account, both the dashboard and `codexm list` show the synthetic `proxy` row with an `@` marker and expose `Last upstream: ...` details. Bare Desktop or CLI launches outside `codexm` are not forced into that path.
