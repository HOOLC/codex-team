...
- If the user wants proxy behavior for one isolated run without writing sessions/auth/config into the live runtime, route them to `codexm run --proxy -- ...codexArgs`; explain that `codexm`-managed proxy entrypoints rewrite both `chatgpt_base_url` and a custom provider, so live Responses websocket turns and REST `/v1` traffic both honor the local proxy, but unmanaged bare `codex` / Desktop launches are outside that guarantee.
