...
- If the user wants Codex or another OpenAI-compatible tool to use one stable local endpoint while `codexm` routes to saved upstream accounts internally, route them to `codexm proxy enable`, `codexm proxy status`, and later `codexm proxy disable`; mention that `codexm switch <name>` updates the proxy's current upstream while proxy remains enabled, autoswitch only moves that upstream after quota-exhaustion signals, both the dashboard and `codexm list` always show the aggregate `proxy` pool row even before proxy mode is enabled, the pool quota/ETA count non-protected eligible accounts with quota snapshots including exhausted ones, and `Proxy last upstream: ...` plus the matching `@` marker only appear while proxy mode is enabled and a recent real upstream hit is known.
