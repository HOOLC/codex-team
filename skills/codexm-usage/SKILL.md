...
- If the user wants Codex or another OpenAI-compatible tool to use one stable local endpoint while `codexm` rotates upstream accounts internally, route them to `codexm proxy enable`, `codexm proxy status`, and later `codexm proxy disable`; mention that the dashboard already shows the aggregate `proxy` pool row even before proxy mode is enabled, that the pool quota/ETA count non-protected eligible accounts with quota snapshots including exhausted ones, and that recent real upstream hits show up as `Proxy last upstream: ...` plus an `@` marker on the matching real upstream row.
