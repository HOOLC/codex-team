...
非 `200` 的 proxy 请求还会额外写入独立的 `proxy-errors-YYYY-MM-DD.jsonl`，里面保留 req/resp 诊断信息。结构化 JSONL 日志始终带 `codexm_version`；设置 `CODEXM_LOG_BUILD_META=1` 后，还会额外记录本地构建元信息，例如 `codexm_git_sha`，方便排查 daemon 和 proxy 实际跑的是哪份代码。
