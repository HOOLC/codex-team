...
Non-200 proxy requests also write a separate `proxy-errors-YYYY-MM-DD.jsonl` file with request/response diagnostics. Structured JSONL logs include `codexm_version` so old runs can be compared against the code that produced them.
