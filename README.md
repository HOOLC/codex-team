...
Non-200 proxy requests also write a separate `proxy-errors-YYYY-MM-DD.jsonl` file with request/response diagnostics. Structured JSONL logs always include `codexm_version`, and proxy request logs also record `request_speed` (`normal` or `fast`); set `CODEXM_LOG_BUILD_META=1` to also record local build metadata such as `codexm_git_sha` when debugging daemon and proxy runs.
